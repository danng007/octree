...
int templateBin::calLevelIndex(int depth, int length, int width) {
	depth = (xmax - depth) / (xmax - xmin)*40;
	length = (ymax - length) / (ymax - ymin)*40;
	width = (zmax - width) / (zmax - zmin)*40;
	arg[depth][length][width]++;
	if (arg[depth][length][width] < maxDepth)
	{
		return arg[depth][length][width];
	}
	else
	{
		return maxDepth;
